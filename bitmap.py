...
def getPixels(path):
    ...
    colorData = [None]*bmpHeight
    for i in range(0, bmpHeight):
        colorData[i] = [None]*bmpWidth

    for j in range(0, bmpHeight):
        for i in range(0, bmpWidth):
            pixel = struct.unpack_from("<BBB", pixelData, bytesPerRow * (bmpHeight - 1 - j) + i * 3)+tuple()

            colorData[j][i] = pixel[::-1]
